...
class PrcmtsController < ApplicationController
  ...
  def new
    ...
    unless item_ids.nil?
      ...
      redirect_to purch_reqn_items_path(@purch_reqn) if PrcmtItem.where(purch_reqn_item_id: item_ids).exists?
    end
  ...
  def create
    ...
    respond_to do |format|
      if @prcmt.save
        items = PurchReqnItem.where(id: prcmt_items_params[:prcmt_items])
        items.each do |item|
          PrcmtItem.create({
                             purch_reqn_item_id: item.id,
                             **item.serializable_hash(except: %i[id created_at updated_at purch_reqn_id processed_qty item_type wbsproject_id parent_id contract_item_reference_id contract_flag purch_reqn_item_noncontract_id commodity_list_id adjustment_qty created_by_id updated_by_id discarded_by_id discarded_at]),
                             prcmt_id: @prcmt.id
                           })
        end
  ...
  def prcmt_items_params
    params.require(:prcmt).permit(prcmt_items: [])
  end
end
